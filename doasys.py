...
from attention_module import ChannelAttention
...
class spectrumModule(nn.Module):
    def __init__(self, signal_dim=8, n_filters=8, n_layers=3, inner_dim=125,
        ...
        self.attention_layer = ChannelAttention(in_channels=n_filters, heads=2, seq_len=inner_dim)
    ...
    def forward(self, inp):
        bsz = inp.size(0)
        inp = inp.view(bsz, -1)  # 展平输入
        x = self.in_layer(inp).view(bsz, self.n_filters, -1)  # 转换为 [batch_size, n_filters, seq_len]
        x = x.permute(0, 2, 1)  # 转换为 [batch_size, seq_len, n_filters] 以适配 Attention
        x = self.attention_layer(x)  # Attention 模块
        x = x.permute(0, 2, 1)  # 转换回 [batch_size, n_filters, seq_len]
        x = self.mod(x)  # 卷积模块
        x = x.view(bsz, -1)  # 展平以适配全连接层
        x = self.out_layer(x).view(bsz, -1)
        return x
