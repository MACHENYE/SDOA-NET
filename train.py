...
if __name__ == '__main__':
    ...
    for idx in range(args.train_num):
        for train_type in range(7):
            ...
            # 数据形状调整，确保数据是 [batch_size, features, seq_len] 或 [batch_size, seq_len, features]
            signal = signal.permute(0, 2, 1)  # 假设 signal 形状是 [batch_size, seq_len, features] 转换为 [batch_size, features, seq_len]
            ...
            # 数据形状调整，确保数据是 [batch_size, features, seq_len] 或 [batch_size, seq_len, features]
            noisy_signals = noisy_signals.permute(0, 2, 1)  # 转换为 [batch_size, features, seq_len]
            ...
            for epoch in range(1, args.n_epochs + 1):
                ...
                # 检查当前验证损失是否为最低
                if loss_val[epoch - 1] < best_val_loss:
                    best_val_loss = loss_val[epoch - 1]
                    torch.save(net, 'best_net.pkl')  # 保存验证损失最低的模型
